...
The project is implemented in Go. The source code is available in the GitHub repository:
\begin{center}
\url{https://github.com/pi128/deadlock-detection-tool}
\end{center}
The command-line entry point is located in \texttt{cmd/deadlock}. The core detection logic is in \texttt{internal/deadlock}, and the sample input files are in \texttt{examples}. I kept the detector in an \texttt{internal} package so the command-line formatting code would not be mixed with the graph and validation logic.
